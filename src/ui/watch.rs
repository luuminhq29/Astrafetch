...
pub fn run(c: Config) -> Result<()> {
    ...
    let result = loop_app(&mut terminal, c);
    disable_raw_mode()?;
    execute!(terminal.backend_mut(), LeaveAlternateScreen)?;
    terminal.show_cursor()?;
    result
}
fn loop_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, c: Config) -> Result<()> {
    let mut paused = false;
    let mut frame_no = 0u64;
    let mut last = Instant::now() - Duration::from_secs(10);
    let mut snap = SystemSnapshot::collect(&c)?;
    loop {
        if !paused && last.elapsed() >= Duration::from_secs(c.refresh_rate.max(1)) {
            snap = SystemSnapshot::collect(&c)?;
            last = Instant::now();
        }
        terminal.draw(|f| renderer::draw(f, &snap, &c, frame_no, paused))?;
        ...
        if event::poll(Duration::from_millis(100))? {
            ...
            }) = event::read()?
            {
                match (code, modifiers) {
                    ...
                    (KeyCode::Char('p'), KeyModifiers::NONE) => paused = !paused,
                    (KeyCode::Char('r'), KeyModifiers::NONE) => {
                        snap = SystemSnapshot::collect(&c)?;
                        last = Instant::now();
                    }
                    (KeyCode::Char('h'), KeyModifiers::NONE) => show_help(terminal)?,
                    (KeyCode::Char('i'), KeyModifiers::NONE) => show_info(terminal, &snap)?,
                    _ => {}
                }
            }
        }
